...
const mapCategoryCard = (category) => {
  return {
    ...
    image: normalizeImageUrl(category?.image_url || category?.image),
  };
};
...
export function Categories() {
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        const data = await categoryAPI.getAll({ category_type: 'shop_by_category' });
        ...
        setCategoryCards(list.map(mapCategoryCard).filter((item) => item.name));
      } catch {
        if (isMounted) {
          setCategoryCards([]);
        }
    ...
    window.addEventListener('categories:updated', handleCategoryUpdates);
    window.addEventListener('focus', handleCategoryUpdates);
    document.addEventListener('visibilitychange', handleVisibilityChange);

    return () => {
      isMounted = false;
      clearInterval(intervalId);
      window.removeEventListener('categories:updated', handleCategoryUpdates);
      window.removeEventListener('focus', handleCategoryUpdates);
  ...
  const isCategoriesExist = categoryCards.length > 0;
