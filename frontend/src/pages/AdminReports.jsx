    ...
    <AdminLayout>
      <div className="mx-auto w-full max-w-7xl space-y-4 sm:space-y-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-[2rem] font-bold text-gray-900 sm:text-3xl">Reports</h1>
          ...
          <div className="flex w-full items-center gap-2 sm:w-auto">
            <select
              ...
              className="h-10 flex-1 rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-semibold text-gray-700 focus:border-green-500 focus:outline-none sm:h-auto sm:min-h-[42px] sm:flex-none"
            >
            ...
            <button
              ...
              className="inline-flex h-10 items-center gap-1.5 rounded-xl border border-gray-200 bg-white px-3 text-sm font-semibold text-gray-700 hover:bg-gray-50 sm:h-auto sm:min-h-[42px] sm:gap-2 sm:px-4 sm:py-2"
            >
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-2.5 shadow-sm">
            <div className="grid grid-cols-2 gap-2">
              {quickSummaryItems.map((item) => (
                <article key={item.key} className="rounded-xl border border-gray-200 bg-gray-50 px-2.5 py-2">
                  <p className="truncate text-[10px] font-semibold uppercase tracking-[0.08em] text-gray-500">
                    {item.label}
                  </p>
                  <p className={`mt-0.5 truncate text-sm font-extrabold ${item.key === "refund" ? "text-red-700" : "text-gray-900"}`}>
                    {item.value}
                  </p>
                </article>
              ))}
        ...
        <div className="hidden grid-cols-2 gap-4 md:grid lg:grid-cols-4">
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Revenue ({daysRange}d)</p>
            <p className="mt-2 text-2xl font-bold text-gray-900">
              {formatPrice(totals.revenue, settings.currency)}
            </p>
          </div>
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Orders ({daysRange}d)</p>
            <p className="mt-2 text-2xl font-bold text-gray-900">{totals.orders}</p>
          </div>
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Average Order Value</p>
            <p className="mt-2 text-2xl font-bold text-gray-900">
              {formatPrice(totals.aov, settings.currency)}
            </p>
          </div>
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Refunded Amount</p>
            ...
            <div className="mb-3 flex items-center justify-between gap-2">
              ...
              <div className="inline-flex rounded-full border border-gray-200 bg-gray-50 p-1">
                <button
                  ...
                  className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${
                    mobileDataView === "sales" ? "bg-[#2a5f1e] text-white" : "text-gray-600"
                  }`}
                >
                  Sales
                </button>
                <button
                  ...
                  className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${
                    mobileDataView === "products" ? "bg-[#2a5f1e] text-white" : "text-gray-600"
                  }`}
                >
                  Products
                </button>
                  ...
                  mobileSalesRows.map((row) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-sm font-semibold text-gray-900">{formatTableDate(row.date)}</p>
        ...
        <div className="hidden grid-cols-1 gap-4 md:grid xl:grid-cols-[58%_42%]">
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
            <div className="mb-4 inline-flex items-center gap-2 text-lg font-bold text-gray-900">
              <FileBarChart2 className="h-5 w-5 text-gray-500" />
                  ...
                  {sortedForTable.length > 0 ? (
                    sortedForTable.map((row) => (
                      <tr key={row.date} className="border-b border-gray-100">
                        ...
                        <td className="py-3 pr-3 text-sm text-gray-700">{Number(row.orders || 0)}</td>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
            <p className="mb-4 text-lg font-bold text-gray-900">Top Product Revenue</p>
