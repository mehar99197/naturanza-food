...
const CoverImage = ({ post, className, emojiClass = 'text-4xl' }) => {
  if (post.image) {
    return (
      // Blog covers are product photographs, and they are taller than the card
      // slot: the ispaghol shot is 602x630 in a 393x176 box. object-cover
      // therefore showed a middle band of roughly 43% of the image, cutting the
      // lid off the top and the base off the bottom. contain on a white field
      // keeps the whole jar, which is also how the category cards present the
      // very same photographs.
