...
/**
 * Floating WhatsApp contact button, ported from
 * frontend/src/components/WhatsAppButton.jsx.
 *
 * "use client" because the number and the on/off switch come from the settings
 * context. Nothing else changes: the link is an external `wa.me` URL, so it
 * stays a plain `<a>` rather than becoming a `next/link` — `next/link` is for
 * in-app navigation and would gain nothing here but prefetch noise.
 *
 * The two early returns are the whole behaviour: an admin can disable the button
 * outright, and a blank/garbage number renders nothing rather than linking to
 * `https://wa.me/` (which resolves to WhatsApp's own landing page and looks like
 * a bug to the visitor).
 */

import { useSettings } from "@/providers/SettingsProvider";

const normalizeWhatsAppNumber = (value: string | null | undefined): string =>
