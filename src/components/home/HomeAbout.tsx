...
/**
 * The "About Us" band on the home page, ported from
 * frontend/src/sections/About.jsx. Named HomeAbout to keep it distinct from the
 * /about route's own sections in @/components/about.
 *
 * A Server Component: every word of this section is a constant, and the only
 * interactive part was the pair of `useScrollReveal` calls, which are now the two
 * `Reveal` wrappers. So the copy and the CTA are in the HTML rather than waiting
 * on hydration — which for a section this static is the whole of its cost.
 *
 * ⚠ PRESERVED: the source's two reveals used `useScrollReveal()` and
 * `useScrollReveal({ threshold: 0.2 })`. 0.2 is the hook's default, so the two
 * calls were already identical and both are plain `Reveal` here.
 */
    ...
    <section className="py-8 sm:py-10 lg:py-14 bg-white">
      <div className="max-w-[1320px] mx-auto px-4 sm:px-6 lg:px-10">
        <div className="grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] gap-6 sm:gap-8 md:gap-10 lg:gap-12 xl:gap-16 items-start lg:items-center">
          {/* Image Side */}
          <Reveal className="relative order-2 lg:order-1 reveal reveal-left">
            <div className="relative h-[220px] sm:h-[320px] md:h-[390px] lg:h-[460px] rounded-2xl md:rounded-3xl overflow-hidden shadow-xl md:shadow-2xl bg-[#eef7ef]">
              {/* eslint-disable-next-line @next/next/no-img-element -- a bundled
                  asset sized entirely by its container; next/image would add a
                  wrapper and change the box this markup depends on. */}
              <img
          ...
          {/* Content Side */}
          <Reveal className="order-1 lg:order-2 space-y-4 sm:space-y-5 md:space-y-6 min-w-0 reveal reveal-right">
            <div>
