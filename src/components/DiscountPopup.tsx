...
/**
 * Store-wide-sale popup, ported from frontend/src/components/DiscountPopup.jsx.
 *
 * Shown once per browser session while a sale is live. The dismissal is recorded
 * as a *signature* (`label|percentage`) rather than a boolean, which is what
 * makes a new sale re-trigger the popup for someone who already dismissed the
 * previous one — change either half in admin and every session sees it again.
 *
 * Routing: the CTA was a react-router `<Link to="/shop">` and is now
 * `next/link`. Same URL.
 *
 * The `typeof window === "undefined"` guard is kept even though effects never
 * run on the server, because it also documents that everything below it is
 * browser-only. `safeSessionStorage` already swallows storage exceptions
 * (private mode, quota); the try/catch around it is belt-and-braces from the
 * original and costs nothing.
 */
